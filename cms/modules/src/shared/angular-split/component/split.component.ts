    ...
    template: `
        ...
        <ng-template ngFor [ngForOf]="displayedAreas" let-index="index" let-last="last">
            <div *ngIf="last === false" 
                 ...
                 (outside-angular-zone.click)="clickGutter($event, index+1)"
                 (outside-angular-zone.mousedown)="startDragging($event, index*2+1, index+1)"
                 (outside-angular-zone.touchstart)="startDragging($event, index*2+1, index+1)">
...
export class SplitComponent implements AfterViewInit, OnDestroy {
    ...
    @Output() get transitionEnd(): Observable<any> {
        return new Observable(subscriber => this.transitionEndSubscriber = subscriber).pipe(debounceTime<Array<number>>(20));
    }
