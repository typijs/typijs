...
export class BlockRouter {
    constructor(private blockController: BlockController, private authGuard: AuthGuard) { }
    ...
    get router(): Router {
        ...
        block.get('/children/:parentId?', this.authGuard.checkRoles(requiredAdminOrEditor), validate(requiredParentId), this.blockController.getContentsByFolder);
        ...
        block.post('/', this.authGuard.checkRoles(requiredAdminOrEditor), validate(insertContent), this.blockController.create);

        block.put('/:id', this.authGuard.checkRoles(requiredAdminOrEditor), validate(requiredContentId), this.blockController.update);
        //publish page
        block.put('/publish/:id', this.authGuard.checkRoles(requiredAdminOrEditor), validate(requiredContentId), this.blockController.publish);
        //move to trash
